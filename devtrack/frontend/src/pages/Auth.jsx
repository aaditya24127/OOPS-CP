...
import { Eye, EyeOff, Code, Terminal, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function Auth() {
  ...
  // Redirect if already logged in securely (and not showing a success toast)
  useEffect(() => {
    if (user && !successToast && !loading) {
      navigate('/', { replace: true });
    }
  }, [user, successToast, loading, navigate]);
  ...
  const handleSignUp = async (e) => {
    ...
    // Register with Supabase (Email confirmations are disabled for this project)
    ...
    // Account created successfully (user is immediately authenticated)
    ...
    // Show success notification and redirect automatically
      ...
      <AnimatePresence>
        {successToast && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            style={{ position: 'absolute', top: '24px', left: '50%', transform: 'translateX(-50%)', backgroundColor: '#10b981', color: 'white', padding: '16px 24px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 10px 25px rgba(0,0,0,0.5)', zIndex: 1000 }}
          >
            <CheckCircle size={24} />
            <span style={{ fontWeight: 'bold' }}>Account created successfully!</span>
          </motion.div>
        )}
      </AnimatePresence>
